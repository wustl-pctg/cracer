...
// made the same types of accesses are considered as the the same race
// (even for races where one is read followed by write and the other is write
...
static RaceMap_t races_found;  // XXX need to protect with lock
...
static void get_info_on_inst_addr(uintptr_t addr, int *line_no, std::string *file) {
    ...
    fprintf(stderr, "%0lx is not in range\n", addr);
}
...
void print_race_report() {
  ...
  std::cerr << std::endl;
  std::cerr << "Race detector detected total of " 
            << races_found.size() << " races." << std::endl;
