...
void Cilk_ds_main(CilkWorkerState *const ws);
void Cilk_scheduler(CilkWorkerState *const ws, Closure *t)
{
  //printf("In Cilk_scheduler.\n");
	int victim, num_main;
	...
	while (!USE_SHARED(done)) {
      ...
      num_main = USE_PARAMETER(num_real_workers);
			// Decide where to steal from -- modified for split BSS45
			if (ws->self >= num_main) { //ds worker
        //printf("ds worker - calling batch scheduler\n");
					//&& !(USE_SHARED(pending_batch).size > MAX_BATCH_SIZE)) {
				  //BSS45 - this condition doesn't make any sense to me, ask rob?
        // data structure steal
				ws->batch_id = USE_SHARED(current_batch_id);
				batch_scheduler(ws, (Closure *) NULL);

			} else { // main worker - regular steal */
				victim = rts_rand(ws) % num_main;//BSS45 - steal only from main workers
				if (victim != ws->self &&
...
{
	int victim;
  int num_ds = USE_PARAMETER(num_ds_workers); //BSS45 move these?
  int num_main = USE_PARAMETER(num_real_workers);
	CILK_ASSERT(ws, ws->self >= 0);
  ...
  //BSS45 - jump to DS main if we're the main ds worker
  //      - this may need to happen sooner for efficiency sake
  if (ws->self == num_main) {
    //printf("ds_main thread, calling Cilk_ds_main\n");
    Cilk_ds_main(ws);
	  ws->batch_id = 0;
	...
	//while (!batch_done_yet(ws, ws->batch_id)) {
		...
		//if (!t && USE_SHARED(current_batch_id) != ws->batch_id) {	
		// finished the batch
      ...
      //BSS45 - modifying victim generation for split version
      //BSS45 - TODO: figure out what Rob is doing here
			if (USE_PARAMETER(options->btest) == 0) {
				//victim = (rts_rand(ws) & num_ds)+num_main;
        victim = (rts_rand(ws)%num_ds) + num_main;
			}	else {
				/*int range = USE_SHARED(pending_batch).size;
				victim = range == 1 ? 0 : rts_rand(ws) % (range-1) + 1;
				victim = USE_SHARED(batch_workers_list)[victim];
        // BSS45 - this assumes only workers with pending batch op
        //         are working on batch? Seems wrong to me*/
        victim = (rts_rand(ws)%num_ds) + num_main;
			}
      //BSS45 - DS_IN_PROGRESS check shouldn't be applicable in split
			if (victim != ws->self) {
    ...
    void *dataStruct, void *data, size_t dataSize, void *indvResult)
{
  ...
  pending->array[ws->self].dataStructure = dataStruct;
  ...
  //BSS45 - in split version main workers should spin while waiting on DS work
  //      - moving batch book keeping and execution elsewhere
  //      - is status fenced/volatile? (check if races)
	while (pending->array[ws->self].status != DS_DONE) {;}
	//printf("pending->array[ws->self].status = %d, returning\n", 
  //  pending->array[ws->self].status);
  return;
}

//BSS45 - seperate method for main DS worker loop
void Cilk_ds_main (CilkWorkerState *const ws) {
	void *workArray;
	int i, numJobs = 0;
  size_t dataSize;
  ...
  void *dataStruct;
	...
	pending = &USE_SHARED(pending_batch);
	ws->batch_id = USE_SHARED(current_batch_id);
  //loop through to find first op, this should probably be optimized later
  /*i=0;
  while (pending->array[i].status != DS_WAITING) {
    i = (i+1) % USE_PARAMETER(active_size);
  }*/
  for (i=0; i < USE_PARAMETER(active_size) 
    && pending->array[i].status != DS_WAITING; ++i) {;}

  if (!pending->array[i].operation) { 
    //printf("no operation\n");
    return; 
  }

  dataSize = pending->array[i].size;
  op = pending->array[i].operation;
  dataStruct = pending->array[i].dataStructure;
	...
	if (Cilk_mutex_try(ws->context, &USE_SHARED(batch_lock))) {
		...
		} else if (pending->dataSize < dataSize) { 
      // last time we used a different operation with different datasize
			Cilk_free(workArray);
			workArray = Cilk_malloc(dataSize * USE_PARAMETER(active_size));
			USE_SHARED(batch_work_array) = workArray;
		}
    ...
    op(ws, dataStruct, workArray, numJobs, workArray);
		...
		//BSS45 - this shouldn't be applicable in split
	...
	//batch_scheduler(ws, NULL); //BSS45 - make this infinite loop instead?
  return;
}
