...
enum {
  NONE, NPROC, DSRATIO, BTEST, STATS, NO_STATS, HELP, STACK, YIELD, NO_YIELD,
  PTHREAD_STACKSIZE,
  ...
  *options = cilk_default_options;
  ...
  for (i = 1; i < *argc; ++i) {
    ...
    case DSRATIO:
      ...
      CHECK(options->dsratio < 1.0, "invalid ds-stealing ratio");
      CHECK(options->dsratio >= 0.0, "invalid ds-stealing ratio");
      break;
    case BTEST:
      ++i;
      CHECK(i < *argc, "argument missing");
