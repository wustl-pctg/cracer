...
rb_red_blk_tree* RBTreeCreate( int (*CompFunc) (const void*,const void*),
  ...
  // Need all the nodes to get an empty linked list
  LinkedList * ll;
  ...
  //Instantiate their linked-lists
  newTree->root->nodes_list = newTree->nil->nodes_list = createLinkedList();
...
/*! This is a simple find function a red-black tree
  \param tree The tree being searched
  \param root The root of the (sub)tree being searched
  \param key The key to be identified
  \return rb_red_blk_node *
*/
rb_red_blk_node * RBTreeFind(rb_red_blk_tree * tree, rb_red_blk_node * root, void * key)
{
  if (root == NULL || root->key == key) return root;
  if (key < root->key) return RBTreeFind(tree, root->left, key);
  else return RBTreeFind(tree, root->right, key);
}
...
rb_red_blk_node * RBTreeInsert(rb_red_blk_tree* tree, void* key, void* info) {
  ...
  // If x's key is already in tree, return it after appending its node
  x = RBTreeFind(tree, tree->root, key);
  if (x!=NULL) {
    appendNode(x->nodes_list, (Node *)info);
    return x;
  }
  ...
  /* Added for the purpose of race_detect */
  x->nodes_list = createLinkedList();
  appendNode(x->nodes_list, (Node *) x->info);
