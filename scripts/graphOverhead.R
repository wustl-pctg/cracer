...
if (is.na(args[1])) {
	 logFile <- "../logs/overhead.357.log"
} else {
	logFile <- args[1]
}
if (is.na(args[2])) {
	 outFile <- "../graphs/overhead.357.dickens.pdf"
} else {
	outFile <- args[2]
}
...
plot(data$NPROC, data$COMPACT, type="o", col="red", ylim=y_range,
     axes = FALSE, ann = FALSE)
lines(data$NPROC, data$FC, type="o", pch=22, lty=2, col="blue")
lines(data$NPROC, data$RAW, type="o", pch=23, lty=3, col="green")
...
legend("topright",
       c("Compaction", "Flat combining", "Raw (separate termination)"),
