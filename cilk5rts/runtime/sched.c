															...
															ReadyDeque *const deque_pool)
{
  ...
  // @todo @opt Why are we waiting on a lock here?
  // Shouldn't we just *try* to get the lock? If we don't immediately
  // get it, just return and try a different victim! But maybe this is
  // not really a bottleneck since it's not on the critical path.
    ...
    // cl->frame == NULL should only happen at the very beginning of a
    // batch function. --rsu
		if (Closure_being_aborted(ws, cl) || cl->frame == NULL) {
														...
														ReadyDeque *const deque_pool)
{
  ...
  /* if (ws->batch_id) { */
  /*   printf("Worker %i suspending a batch closure.\n", ws->self); */
  /* } */
...
int Cilk_sync(CilkWorkerState *const ws)
{
    ...
    //    if (ws->batch_id) printf("Worker %i suspending a batch closure.\n", ws->self);
...
int Cilk_exception_handler(CilkWorkerState *const ws, void *resultp, int size)
{
  ...
  /* if (ws->batch_id) { */
  /*   printf("Batch exception in worker %i\n", ws->self); */
  /* } */
...
void Cilk_set_result(CilkWorkerState *const ws, void *resultp, int size)
{
	...
	deque_lock(ws, ws->self, ws->current_deque_pool);
	t = deque_peek_bottom(ws, ws->self, ws->current_deque_pool);
  if (t == NULL && ws->batch_id) {
    deque_unlock(ws, ws->self, ws->current_deque_pool);
    return;
    //    printf("Worker %i was in slow code, but now has no Closure on deque!\n", ws->self);
  }
	Closure_lock(ws, t);
...
// @bug Right now workers are spending way too much time in the batch
// scheduler. It seems like they are stealing a closure, but either
// they have stayed way too long in the steal loop, or the closure is
// modified after the steal. So when they execute the closure, their
// batch is already very much completed.
// Basically, workers are stealing INTO the critical path, putting the
// cost of the steals on the critical path.
  ...
  while (!batch_done_yet(ws, ws->batch_id)) {
    ...
    if (!t && batch_sleep(ws, USE_PARAMETER(sleeptime), batch_id)) break;
      ...
      should_steal = (rts_rand(ws) % 99) + 1;
        ...
        if (batch_done_yet(ws, ws->batch_id)) break;
          ...
          /* if (t) { */
          /*   printf("Worker %i stole batch closure from %i\n", ws->self, victim); */
          /* } */
...
void Cilk_set_stage(CilkWorkerState* const ws, int stage)
{
  CILK_ASSERT(ws, USE_PARAMETER(stage_lock) == stage - 1);
  USE_PARAMETER(stage_lock) = stage;
}

void Cilk_wait_for_stage(CilkWorkerState* const ws, int stage)
{
  while (USE_PARAMETER(stage_lock) < stage) { }
}
...
void invoke_batch(CilkWorkerState *const ws, InternalBatchOperation op,
                  void* ds, void* work_array, unsigned int num_ops)
{
  ...
  op(ws, ds, work_array, num_ops, NULL);

  deque_lock(ws, ws->self, USE_PARAMETER(ds_deques));
  t1 = deque_peek_bottom(ws, ws->self, USE_PARAMETER(ds_deques));
  if (t1 == t) {
									 ...
									 void *data, size_t data_size, void *indv_result)
{
  ...
  do {
    ...
    batch_id = USE_SHARED(current_batch_id);
      ...
      invoke_batch(ws, op, ds, work_array, num_ops);
